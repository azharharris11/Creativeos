...
import { RefreshCwIcon, SparklesIcon, InfoIcon, BrushIcon, ChevronLeftIcon } from './icons';
...
export const EditModal: React.FC<EditModalProps> = ({ concept, campaignBlueprint, onSave, onClose, onGenerateImage }) => {
  ...
  const [history, setHistory] = useState<AdConcept[]>([]); // History Stack
  ...
  const pushToHistory = (currentData: AdConcept) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    pushToHistory(formData); // Save state before change
      ...
      <div className="bg-brand-surface rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <header className="p-4 border-b border-gray-700 flex-shrink-0 flex justify-between items-center">
          <div>
              ...
              <p className="text-sm text-brand-text-secondary truncate max-w-md">Ad Set: {formData.adSetName}</p>
          </div>
          <div className="flex items-center gap-2">
               <button 
                  onClick={handleUndo} 
                  disabled={history.length === 0}
                  className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-white text-sm font-semibold rounded-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
                  title="Undo changes (Ctrl+Z)"
              >
                  <ChevronLeftIcon className="w-4 h-4" /> Undo
              </button>
          </div>
        </header>
