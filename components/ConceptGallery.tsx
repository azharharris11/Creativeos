...
import { 
    ...
    RocketIcon, HomeIcon, UserSquareIcon, LayoutGridIcon, NetworkIcon, FilterIcon
} from './icons'; 
...
export const ConceptDashboard: React.FC<ConceptDashboardProps> = (props) => {
  ...
  const filteredConceptGroups = useMemo(() => {
    ...
    filteredConcepts.forEach(concept => {
      ...
      // Group by angle -> trigger -> format -> placement path
    ...
    return Object.entries(personaGroups).map(([personaDesc, data]) => {
      const hypotheses = Object.entries(data.hypothesisGroups).map(([pathId, groupConcepts]) => {
        const firstConcept = groupConcepts[0];
        let title = `${firstConcept.angle} → ${firstConcept.trigger.name} → ${firstConcept.format}`;
        return { pathId, concepts: groupConcepts, title };
      });
      return { personaDesc, persona: data.persona, hypotheses };
    }).sort((a,b) => a.personaDesc.localeCompare(b.personaDesc));
  ...
  // --- Render Component ---
  return (
    <div className="w-full min-h-full flex bg-gray-950 text-white font-sans">

      {/* --- Sidebar (Baru) --- */}
      <aside className="w-72 flex-shrink-0 bg-gray-900/80 backdrop-blur-lg border-r border-gray-800 flex flex-col fixed top-[60px] left-0 h-[calc(100vh-60px)] z-30">
        
        <header className="p-6 border-b border-gray-800">
          <h1 className="text-2xl font-black text-indigo-400 uppercase tracking-wider">
            KreatifOS
          </h1>
          <p className="text-sm text-gray-400">Dasbor Eliminasi</p>
        </header>
      ...
      <main className="flex-1 flex flex-col ml-72">
            ...
            <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-4 flex-grow">
                    ...
                    <FilterControls 
                        nodes={nodes} 
                        concepts={concepts}
                        onFilterChange={(newFilters: Partial<GalleryFilters>) => setFilters(f => ({...f, ...newFilters}))} 
                    />
                </div>
        ...
        <div className="flex-grow overflow-y-auto p-6 space-y-8">
            {selectedConcepts.size > 0 && (
                <div className="sticky top-[0.5rem] z-10 mb-6 p-4 bg-indigo-900/90 backdrop-blur-sm rounded-xl shadow-2xl border border-indigo-700/50 transition-all duration-300">
            ...
            {filteredConceptGroups.length > 0 ? (
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4">
                                            {groupConcepts.map(concept => {
                                                return (
                                                    <CreativeCard 
                                                        key={concept.id} 
                                                        node={{ id: concept.id, type: 'creative', label: concept.headline, content: { concept }, position: {x:0, y:0}}}
                ...
                <div className="flex flex-col items-center justify-center h-[50vh] text-gray-400 p-8 border-2 border-dashed border-gray-700 rounded-2xl bg-gray-900/30">
                    <div className="p-4 bg-gray-800 rounded-full mb-4 shadow-lg">
                         <FilterIcon className="w-10 h-10 text-gray-500"/>
                    </div>
                    <h3 className="text-xl font-bold text-gray-200">Tidak ada konsep yang cocok dengan filter.</h3>
                    <p className="mt-2 text-sm max-w-md text-center">Filter Anda mungkin terlalu ketat. Coba atur ulang filter untuk melihat semua konsep yang tersedia.</p>
                    <button 
                        onClick={() => setFilters({angle: 'all', persona: 'all', format: 'all', trigger: 'all', campaignTag: 'all', statusTag: 'all'})}
                        className="mt-6 px-6 py-2 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-500 transition-colors shadow-md flex items-center gap-2"
                    >
