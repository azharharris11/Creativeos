...
// --- Ikon ---
import { 
    EditIcon, ClipboardCopyIcon, RefreshCwIcon, DownloadIcon, ChevronLeftIcon, ChevronRightIcon,
...
// --- Komponen Skeleton Loader (Baru) ---
const SkeletonLoader = () => (
    <div className="w-full h-full absolute inset-0 bg-gray-900 animate-pulse flex flex-col">
        <div className="w-full h-full bg-gray-800 flex items-center justify-center">
             <div className="text-center opacity-50">
                 <SparklesIcon className="w-8 h-8 mx-auto mb-2 text-gray-600 animate-bounce" />
                 <p className="text-xs text-gray-500 font-mono">AI sedang bekerja...</p>
             </div>
        </div>
        {/* Overlay Skeleton imitating text zones */}
...
export const CreativeCard: React.FC<CreativeCardProps> = ({ node, onGenerateImage, onEditConcept, onOpenLightbox, isSelected, onSelect, onSaveConcept, className = '' }) => {
  ...
  const handleEditToggle = (e: React.MouseEvent) => {
    ...
    if(isEditing) { // If was editing, reset text on cancel
        setEditText({ hook: concept.hook, headline: concept.headline });
    }
  };
  ...
  const handleSave = (e: React.MouseEvent) => {
    ...
    if (isCarousel && concept.carouselSlides) {
        ...
        if (slideToUpdate && (slideToUpdate.hook !== editText.hook || slideToUpdate.headline !== editText.headline)) {
            ...
            const updatedConcept = currentSlide === 0 
                ? { ...concept, hook: editText.hook, headline: editText.headline, carouselSlides: updatedSlides }
                : { ...concept, carouselSlides: updatedSlides };
  ...
  return (
    <div className={`bg-gray-800 border rounded-xl shadow-xl flex flex-col transition-all duration-300 overflow-hidden w-full h-full ${isSelected ? 'border-indigo-600 ring-2 ring-indigo-500' : 'border-gray-700'} ${className}`}>
      ...
      <div className="relative w-full aspect-square flex-shrink-0 group bg-gray-900">
        ...
        {concept.isGenerating ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center text-white bg-gray-800/50">
            <SparklesIcon className="w-8 h-8 text-gray-600 mb-2" />
            <p className="text-gray-400 text-xs mb-3 font-medium">Belum ada visual</p>
            <button onClick={(e) => { e.stopPropagation(); onGenerateImage(node.id); }} className="px-4 py-2 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-500 transition-colors text-sm shadow-lg">
      ...
      <div className="p-3 flex-grow flex flex-col justify-between">
        ...
        {isEditing ? (
          <div className="flex-grow flex flex-col">
            ...
            <label htmlFor={`headline-${node.id}`} className="mt-2 text-xs font-semibold text-indigo-400 mb-1">Headline</label>
            <textarea
              id={`headline-${node.id}`}
